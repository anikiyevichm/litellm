...
def test_destination_sets_endpoint_and_headers_on_owned_exporter_only():
    cache = _cache(
        "langfuse_otel",
        exporters=[
            ExporterSpec(
            ...
            ExporterSpec(kind="in_memory", owner="langfuse_otel"),
        ],
    )
    ...
    assert otlp.endpoint == "https://cloud.langfuse.com/api/public/otel"
...
def test_request_credentials_are_inert_on_v2():
    """Langfuse credentials in the request's dynamic params (no admin destination)
    must NOT produce a per-tenant destination, so the trace is not redirected."""
