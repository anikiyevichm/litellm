...
const LoggingSettings: React.FC<LoggingSettingsProps> = ({
  ...
  // Named credentials the admin can bind as an OTEL destination (host + keys live
  // in the registry, never in the request). Sourced from the same registry models use.
  const { data: credentialsResponse } = useCredentials();
  const credentialNames = (credentialsResponse?.credentials ?? []).map(
